...
- 回复形态由目录声明：`reply=text`（默认）回复文本；`reply=image_url` 只在无参数查询时通过
  渠道 Adapter 的原生图片接口发送；`reply=image_url_always` 在图片与参数化查询结果一一对应时
  也优先发送原生图片。服务未返回图片链接时回退为文本；`image_url` 的带参数查询回退为文本加
  链接，避免图片内容与筛选条件不一致。
