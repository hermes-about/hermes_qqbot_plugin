...
的 `send_image` 发送原生图片。Adapter 不支持图片或上传失败时回退成图片 URL 文本。带参数筛选时
仍回复文本加图片链接（图片无法体现筛选结果），服务没返回图片链接时也会回退成文本，避免发出
空消息。默认值 `"reply": "text"` 表示始终回复文本。图片来源只接受 `http` 或 `https` URL。
